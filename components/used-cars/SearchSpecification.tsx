...
import { Modal, ModalBody } from '../common/SideModal';
  ...
  const prepareData = useCallback(
    (searchKey: string | null) => {
      if (specifications && features) {
        ...
        if (filteredFeatures && filteredFeatures.length > 0 && showFeatures) {
        ...
        <div>
          <div className="flex gap-4 items-center">
            ...
            <p className="uppercase text-2xl text-primary">
              {t(LabelConstants.SPECIFICATIONS)}
            </p>
          </div>
          <div className="flex bg-lighter-gray w-full rounded mt-5 items-center h-16">
            <input
              placeholder={t(LabelConstants.SEARCH_SPECIFICATIONS)}
              className="border-none bg-lighter-gray"
              onChange={(e: any) => handleSearch(e)}
            />
            <SearchIcon className="w-10 h-5" />
          </div>
          <div className="flex flex-row border-l-[5px]  my-8 -ml-8 w-13/12">
            <div className="w-80">
              {category.map((x, i) => (
                <div
