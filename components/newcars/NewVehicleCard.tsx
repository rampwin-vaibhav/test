...
const NewVehicleCard = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (
        vehicle.VehicleListingId === localStorageVehicleId ||
        vehicle.ProductCatalogueId === localStorageVehicleId
      ) {
        bookmarkHandler();
      }
    }
  ...
  const handleButtonClick = (vehicle: SearchAllResultItem) => {
    router.push(
      `/newcars/new/info/${vehicle.VehicleMakeKey.toLowerCase()}/${vehicle.VehicleModelKey.toLowerCase()}${
        vehicle.Type === ProductType.VehicleListing
          ? `?v=${CommonUtils.encode(vehicle.VehicleListingId)}`
          : `?p=${CommonUtils.encode(vehicle.ProductCatalogueId)}`
      }`
    );
  };
  ...
  return (
    <div className="gogo-vehicle-card !flex !flex-col card-shadow">
      <div className="relative aspect-[16/9] w-full border-b-2">
        <Link
          href={`/newcars/new/info/${encodeURIComponent(
            vehicle.VehicleMakeKey.toLowerCase()
          )}/${encodeURIComponent(vehicle.VehicleModelKey.toLowerCase())}${
            vehicle.Type === ProductType.VehicleListing
              ? `?v=${CommonUtils.encode(vehicle.VehicleListingId)}`
              : `?p=${CommonUtils.encode(vehicle.ProductCatalogueId)}`
          }`}
        ...
        {vehicle.IsGGMInspected === SearchBoolean.True && (
