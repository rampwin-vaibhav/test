...
const ModelFilter = ({ models, appliedFilter, onChange }: ModelFilterProps) => {
  ...
  const handleLocalChange = (event: Model, isSelect: boolean) => {
    if (isSelect) {
      setSelectedModels([...(selectedModels || []), event.ModelKey]);
    } else {
      setSelectedModels(
        (selectedModels || []).filter((x) => x !== event.ModelKey)
      );
    }
  };
    ...
    if (applyFilter) {
      onChange({
        ...
        list: models.filter((x) =>
          selectedModels.some(
            (y) => String(y).toLowerCase() === String(x.ModelKey).toLowerCase()
          )
        ),
        isSelected: true,
      });
                  ...
                  {models
                    .filter(
                      (x: Model, index: number) =>
                        (!searchKey ||
                          x.ModelKey.toLowerCase().includes(
                            searchKey.toLowerCase()
                          )) &&
                        index ===
                          models.findIndex(
                            (o) =>
                              x.ModelKey.toLowerCase() ===
                              o.ModelKey.toLowerCase()
                          )
                    )
                    .sort((a, b) =>
