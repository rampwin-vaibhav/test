...
const SpecFilter = ({ specs, appliedFilter, onChange }: SpecFilterProps) => {
  ...
  const handleLocalChange = (event: Spec, isSelect: boolean) => {
    if (isSelect) {
      setSelectedSpecs([...(selectedSpecs || []), event.TrimKey]);
    } else {
      setSelectedSpecs(
        (selectedSpecs || []).filter((x) => x !== event.TrimKey)
      );
    }
  };
  ...
  const applySpecFilter = (applyFilter: boolean) => {
    if (applyFilter) {
      onChange({
        ...
        list: specs.filter((x) =>
          selectedSpecs.some(
            (y) => String(y).toLowerCase() === String(x.TrimKey).toLowerCase()
          )
        ),
        isSelected: true,
      });
                ...
                <div className="mt-5 pr-2 grid grid-cols-2 md:grid-cols-4 w-full gap-2">
                  {specs
                    .filter(
                      (x: Spec, index: number) =>
                        (!searchKey ||
                          x.TrimKey.toLowerCase().includes(
                            searchKey.toLowerCase()
                          )) &&
                        index ===
                          specs.findIndex(
                            (o) =>
                              x.TrimKey.toLowerCase() ===
                              o.TrimKey.toLowerCase()
                          )
                    )
                    .sort((a, b) =>
