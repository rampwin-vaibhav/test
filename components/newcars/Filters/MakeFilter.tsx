...
const MakeFilter = ({ makes, appliedFilter, onChange }: MakeFilterProps) => {
  ...
  const handleLocalChange = (event: Make, isSelect: boolean) => {
    if (isSelect) {
      setSelectedMakes([...(selectedMakes || []), event.MakeKey]);
    } else {
      setSelectedMakes(
        (selectedMakes || []).filter((x) => x !== event.MakeKey)
      );
    }
  };
  ...
  const applyMakeFilter = (applyFilter: boolean) => {
    if (applyFilter) {
      onChange({
        ...
        list: makes.filter((x) =>
          selectedMakes.some(
            (y) => String(y).toLowerCase() === String(x.MakeKey).toLowerCase()
          )
        ),
        isSelected: true,
      });
                ...
                <div className="mt-5 pr-2 grid grid-cols-2 md:grid-cols-4 w-full gap-2">
                  {makes
                    .filter(
                      (x: Make) =>
                        !searchKey ||
                        x.MakeKey.toLowerCase().includes(
