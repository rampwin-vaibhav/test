 ...
 * @returns JSX.Element
 */
const FormInputV1 = <T extends FieldValues>({
  name,
  control,
  showError = true,
  label,
        ...
        return (
          <div className="w-full flex flex-col">
            ...
            {showUnit && unitText ? (
              ...
              <div
                className={`rounded-[8px] border border-[#484E50] border-opacity-[30%] focus-within:border-opacity-100 transition-opacity min-h-[48px] flex items-center justify-between px-[16px] ${
                  disabled && 'bg-[#E0E0E0]'
                }`}
              >
                {startIcon && startIcon}
                <input
                  ...
                  placeholder={placeholder}
                  disabled={disabled}
                  className="h-[48px] p-0 border-none rounded-[8px]"
                  aria-invalid={
                    showError &&
                    (isDirty || isSubmitted) &&
                    !isValid &&
