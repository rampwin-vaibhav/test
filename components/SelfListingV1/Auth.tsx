...
const Auth = () => {
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data: IFormInput) => {
    if (data.OTPNumber) {
      ...
      if (cleverTap) {
        cleverTap.event?.push('sl_user_authenticated');
      }
  ...
  const handleSendOTP = async () => {
    ...
    if (valid) {
      ...
      if (otpRes) {
        ...
        if (otpSentOnce) {
          if (cleverTap) {
            cleverTap.event?.push('sl_auth_otp_resent');
          }
        } else {
          if (cleverTap) {
            cleverTap.event?.push('sl_auth_otp_sent');
          }
          setOtpSentOnce(true);
        }
      }
  ...
  useEffect(() => {
    if (cleverTap) {
      cleverTap.event?.push('sl_auth_intiated');
    }
  }, [cleverTap]);

  return (
    <div className="ml-[32px] mr-[20px] rtl:mr-[32px] rtl:ml-[20px]">
